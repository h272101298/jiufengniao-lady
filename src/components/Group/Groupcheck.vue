<template>
  <el-row class="warp" style="padding:20px 0 0 20px;">
    ...
    <el-col :span="24" class="warp-main">
      ...
      <el-table :data="list" border stripe style="width:98%" size="small">
...
<script>
  ...
  import { Groupcheck } from '../../api/api';
  ...
  export default {
    ...
    methods:{
      ...
      handleEdit(index, row){
        console.log(row.id)
        sessionStorage.setItem('Groupcheckid', row.id);
        this.$router.push({ path: '/Group/Groupchange' });
      },

      handlePass(index, row){
        var allParams = '?id='+row.id+'&state=2';
        Groupcheck(allParams).then((res) => {
          console.log(res)

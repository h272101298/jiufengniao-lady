 ...
 <el-row class="warp" style="padding:20px 0 0 20px;">
  ...
  <el-col :span="24" class="warp-main">
    <el-row>
      ...
      <el-form ref="baseset" :model="baseset" label-width="150px" class="form" status-icon size="small">
        <h3>短信设置：</h3>

        <el-form-item label="阿里大鱼appkey：">
          <el-input v-model="baseset.upsize" placeholder="请输入业务名称" v-show="show"></el-input>
          <label v-show="set">{{baseset.upsize}}</label>
        </el-form-item>

        <el-form-item label="阿里大鱼secretkey：">
          <el-input v-model="baseset.upsize" placeholder="请输入业务名称" v-show="show"></el-input>
          <label v-show="set">{{baseset.upsize}}</label>
        </el-form-item>

        <el-form-item label="公司名/品牌名：">
          <el-input v-model="baseset.upsize" placeholder="请输入业务名称" v-show="show"></el-input>
          <label v-show="set">{{baseset.upsize}}</label>
        </el-form-item>
        ...
        <el-form-item label="注册启用短信：">
          <el-input v-model="baseset.upsize" placeholder="请输入业务名称" v-show="show"></el-input>
          <label v-show="set">{{baseset.upsize}}</label>
        </el-form-item>

        <el-form-item label="短信码超时时间：">
          <el-input v-model="baseset.upsize" placeholder="请输入业务名称" v-show="show"></el-input>
          <label v-show="set">{{baseset.upsize}}</label>
        </el-form-item>
